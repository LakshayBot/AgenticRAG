...
async def _prepare_chunks_and_sources(
    ...
    with rag_tracer.trace_search(trace, request.query, request.top_k) as search_span:
        ...
        for hit in search_results.get("hits", []):
            ...
            if ghsa_id:
                source_ids.append(ghsa_id)
                sources_set.add(f"https://github.com/advisories/{ghsa_id}")
            elif cve_id:
                sources_set.add(f"https://www.cve.org/CVERecord?id={cve_id}")
            elif source_id:
                source_ids.append(source_id)

        # End search span with essential metadata
        rag_tracer.end_search(search_span, chunks, source_ids, search_results.get("total", 0))

    return chunks, list(sources_set), source_ids
