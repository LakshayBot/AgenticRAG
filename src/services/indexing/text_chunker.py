...
class TextChunker:
    ...
    def chunk_paper(
        ...
        """Chunk a document using hybrid section-based approach.
        ...
        :param source_id: Source identifier (upload_id)
        ...
        if sections:
            try:
                section_chunks = self._chunk_by_sections(title, abstract, source_id, sections)
                if section_chunks:
                    logger.info(f"Created {len(section_chunks)} section-based chunks for {source_id}")
                    return section_chunks
            except Exception as e:
                logger.warning(f"Section-based chunking failed for {source_id}: {e}")

        # Fallback to traditional word-based chunking
        logger.info(f"Using traditional word-based chunking for {source_id}")
        return self.chunk_text(full_text, source_id)
    ...
    def chunk_text(self, text: str, source_id: str) -> List[TextChunk]:
        ...
        if len(words) < self.min_chunk_size:
            logger.warning(f"Text for source {source_id} has only {len(words)} words, less than minimum {self.min_chunk_size}")
        ...
        logger.info(f"Chunked source {source_id}: {len(words)} words -> {len(chunks)} chunks")

        return chunks

    def _chunk_by_sections(
        self, title: str, abstract: str, source_id: str, sections: Union[Dict[str, str], str, list]
    ) -> List[TextChunk]:
        ...
        if not sections_dict:
            logger.warning(f"No meaningful sections found after filtering for {source_id}")
            return []
        ...
        for i, (section_title, section_content) in enumerate(section_items):
            ...
            if section_words < 100:
                ...
                if i == len(section_items) - 1 or len(str(section_items[i + 1][1]).split()) >= 100:
                    chunks.extend(self._create_combined_chunk(header, small_sections, chunks, source_id))
                    small_sections = []

            elif 100 <= section_words <= 800:
                # Perfect size - create single chunk
                chunk_text = f"{header}Section: {section_title}\n\n{content_str}"
                chunk = self._create_section_chunk(chunk_text, section_title, len(chunks), source_id)
                chunks.append(chunk)
                ...
                section_chunks = self._split_large_section(full_section_text, header, section_title, len(chunks), source_id)
    ...
    def _create_combined_chunk(self, header: str, small_sections: List, existing_chunks: List, source_id: str) -> List[TextChunk]:
        ...
        chunk = self._create_section_chunk(combined_text, combined_title, len(existing_chunks), source_id)
        return [chunk]

    def _create_section_chunk(self, chunk_text: str, section_title: str, chunk_index: int, source_id: str) -> TextChunk:
    ...
    def _split_large_section(
        self, full_section_text: str, header: str, section_title: str, base_chunk_index: int, source_id: str
    ) -> List[TextChunk]:
        ...
        traditional_chunks = self.chunk_text(section_only, source_id)
    ...
    def chunk_advisory(
        ...
        affected_ecosystems: Optional[List[str]] = None,
        cwe_ids: Optional[List[str]] = None,
    ) -> List[TextChunk]:
        """Chunk a GitHub Security Advisory into searchable text segments.

        Strategy:
        - Chunk 0: Summary + key metadata (GHSA ID, severity, CVE, CVSS)
        - Chunk 1+: Description chunks (split if description is long)
        - Last chunk: Technical details (packages, ecosystems, CWEs)

        Args:
            ghsa_id: GitHub Security Advisory ID (e.g., GHSA-xxxx-xxxx-xxxx)
            advisory_id: Database ID of the advisory
            ...
            affected_ecosystems: List of affected ecosystems (npm, pip, go, etc.)
        ...
        # Chunk 0: Summary + Key Metadata
        summary_lines = [f"GitHub Security Advisory: {ghsa_id}"]

        if severity:
            summary_lines.append(f"Severity: {severity.upper()}")

        if cve_id:
            summary_lines.append(f"CVE: {cve_id}")

        if cvss_score is not None:
            summary_lines.append(f"CVSS Score: {cvss_score}")

        if affected_ecosystems:
            summary_lines.append(f"Affected Ecosystems: {', '.join(affected_ecosystems)}")
        ...
        # Chunk 1+: Description (split if long)
        if description and description.strip():
            description_words = self._split_into_words(description)

            # If description is short, create single chunk
            if len(description_words) <= self.chunk_size:
                ...
                # Split long descriptions into multiple chunks
                current_position = 0
                part_number = 1

                while current_position < len(description_words):
                    chunk_end = min(current_position + self.chunk_size, len(description_words))
                    chunk_words = description_words[current_position:chunk_end]
                    chunk_text = f"Description (Part {part_number}):\n\n{self._reconstruct_text(chunk_words)}"

                    overlap_with_previous = min(self.overlap_size, current_position) if current_position > 0 else 0
                    overlap_with_next = self.overlap_size if chunk_end < len(description_words) else 0
                    ...
                    chunks.append(
                        TextChunk(
                            ...
                            metadata=ChunkMetadata(
                                chunk_index=chunk_index,
                                start_char=len(" ".join(description_words[:current_position])) if current_position > 0 else 0,
                                end_char=len(" ".join(description_words[:chunk_end])),
                                word_count=len(chunk_words),
                                overlap_with_previous=overlap_with_previous,
                                overlap_with_next=overlap_with_next,
                                section_title=f"Description (Part {part_number})",
                            ),
                            ghsa_id=ghsa_id,
                            advisory_id=advisory_id,
                        )
        ...
        # Last chunk: Technical Details (packages, CWEs)
        technical_lines = []

        if affected_packages:
            technical_lines.append("Affected Packages:")
            for package in affected_packages[:20]:  # Limit to first 20 packages
                technical_lines.append(f"  - {package}")
            if len(affected_packages) > 20:
                technical_lines.append(f"  ... and {len(affected_packages) - 20} more packages")

        if cwe_ids:
            technical_lines.append("\nCommon Weakness Enumeration (CWE):")
            for cwe in cwe_ids[:10]:  # Limit to first 10 CWEs
                technical_lines.append(f"  - {cwe}")
            if len(cwe_ids) > 10:
                technical_lines.append(f"  ... and {len(cwe_ids) - 10} more CWEs")
            ...
            chunks.append(
                TextChunk(
                    ...
                    metadata=ChunkMetadata(
                        chunk_index=chunk_index,
                        start_char=0,
                        end_char=len(technical_text),
                        word_count=len(technical_text.split()),
                        overlap_with_previous=0,
                        overlap_with_next=0,
        ...
        logger.info(f"Chunked advisory {ghsa_id}: {len(chunks)} chunks created")
        return chunks
