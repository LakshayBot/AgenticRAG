...
class LangfuseTracer:
    ...
    def __init__(self, settings: Settings):
        ...
        if self.settings.enabled and self.settings.public_key and self.settings.secret_key:
            try:
                ...
                logger.info(f"Langfuse v3 tracing initialized (host: {self.settings.base_url})")
    ...
    def create_span(
        ...
        Args:
            trace: Langfuse trace object (from client.trace())
            name: Name for this span
            input_data: Input to this operation
            metadata: Additional metadata

        Returns:
            Span object, or None if Langfuse is disabled or trace is None
        """
    ...
    def end_span(
        ...
        """
        End a span, optionally updating it with output/metadata first.
