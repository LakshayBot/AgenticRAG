...
class RAGPromptBuilder:
    ...
    def _load_system_prompt(self) -> str:
        ...
        if not prompt_file.exists():
            ...
            return (
                ...
                "GitHub Security Advisories and uploaded PDF documents. Base your answer STRICTLY on the provided "
                "document excerpts."
            )
    ...
    def create_rag_prompt(self, query: str, chunks: List[Dict[str, Any]]) -> str:
        ...
        for i, chunk in enumerate(chunks, 1):
            ...
            source_id = chunk.get("source_id", chunk.get("ghsa_id", ""))

            # Only include minimal metadata - just source_id for citation
            prompt += f"[{i}. Source:{source_id}]\n"
            prompt += f"{chunk_text}\n\n"

        prompt += f"### Question:\n{query}\n\n"
        prompt += (
            "### Answer:\nProvide a natural, conversational response (not JSON) and cite sources using [Source:id] format.\n\n"
        )
