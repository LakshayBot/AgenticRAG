...
from .index_config_hybrid import SECURITY_RAG_CHUNKS_MAPPING, HYBRID_RRF_PIPELINE
...
class OpenSearchClient:
    ...
    def _create_hybrid_index(self, force: bool = False) -> bool:
        ...
        try:
            ...
            if not self.client.indices.exists(index=self.index_name):
                self.client.indices.create(index=self.index_name, body=SECURITY_RAG_CHUNKS_MAPPING)
    ...
    def search_papers(
        ...
        return self._search_bm25_only(
            query=query, size=size, from_=from_, categories=categories, source_ids=source_ids, latest=latest
        )
    ...
    def search_chunks_vector(
        ...
        try:
            ...
            if source_ids:
                filter_clause.append({"terms": {"source_id": source_ids}})
    ...
    def _search_bm25_only(
        self, query: str, size: int, from_: int, categories: Optional[List[str]], source_ids: Optional[List[str]], latest: bool
    ) -> Dict[str, Any]:
    ...
    def _search_hybrid_native(
        ...
        knn_query = {"knn": {"embedding": {"vector": query_embedding, "k": size * 2}}}

        # Apply the same filters to KNN query
        if categories or source_ids:
            filter_clause = []
            if categories:
                filter_clause.append({"terms": {"categories": categories}})
            if source_ids:
            ...
            # Wrap KNN in a bool query with filters
            knn_query = {"bool": {"must": [knn_query], "filter": filter_clause}}
    ...
    def delete_upload_chunks(self, upload_id: str) -> bool:
        """Delete all chunks for a specific uploaded PDF.

        :param upload_id: Upload ID of the PDF
        :returns: True if deletion was successful
        """
        try:
            response = self.client.delete_by_query(
                index=self.index_name, body={"query": {"term": {"source_id": upload_id}}}, refresh=True
            )

            deleted = response.get("deleted", 0)
            logger.info(f"Deleted {deleted} chunks for upload {upload_id}")
            return deleted > 0
    ...
    def delete_advisory_chunks(self, ghsa_id: str) -> bool:
        """Delete all chunks for a specific GitHub Security Advisory.
        ...
        try:
            response = self.client.delete_by_query(
                index=self.index_name, body={"query": {"term": {"ghsa_id": ghsa_id}}}, refresh=True
            )

            deleted = response.get("deleted", 0)
            logger.info(f"Deleted {deleted} chunks for advisory {ghsa_id}")
            return deleted > 0

        except Exception as e:
            logger.error(f"Error deleting advisory chunks: {e}")
            return False

    def get_chunks_by_advisory(self, ghsa_id: str) -> List[Dict[str, Any]]:
        """Get all chunks for a specific GitHub Security Advisory.

        :param ghsa_id: GHSA ID of the advisory (e.g. "GHSA-xxxx-yyyy-zzzz")
        ...
        try:
            ...
            logger.info(f"Found {len(chunks)} chunks for advisory {ghsa_id}")
            return chunks

        except Exception as e:
            logger.error(f"Error getting advisory chunks: {e}")
            return []

    def get_chunks_by_upload(self, upload_id: str) -> List[Dict[str, Any]]:
        """Get all chunks for a specific uploaded PDF.
