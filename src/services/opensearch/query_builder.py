...
class QueryBuilder:
    ...
    def __init__(
        ...
        :param source_ids: Filter by specific source IDs (upload_id or ghsa_id)
        ...
        if fields is None:
            if search_chunks:
                # Search both 'chunk_text' and 'content' (uploaded PDFs)
                self.fields = ["chunk_text^3", "content^3", "title^2", "abstract^1"]
            else:
    ...
    def _build_filters(self) -> List[Dict[str, Any]]:
        ...
        if self.source_ids:
            filters.append({"terms": {"source_id": self.source_ids}})
    ...
    def _build_source_fields(self) -> Any:
        ...
        if self.search_chunks:
            ...
            return ["source_id", "title", "authors", "abstract", "categories", "published_date", "pdf_url"]
