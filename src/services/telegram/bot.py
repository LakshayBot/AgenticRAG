...
class TelegramBot:
    ...
    async def _start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            "Welcome to the Security RAG Assistant!\n\n"
            "Ask me questions about GitHub Security Advisories or uploaded PDFs.\n\n"
            "Commands:\n"
            "/help - Show this help\n"
    ...
    async def _help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle /help command."""
        await update.message.reply_text(
            "Send me any question about security advisories or uploaded documents.\n\n"
            "Examples:\n"
            "- What is GHSA-xxxx-yyyy-zzzz?\n"
            "- How does this vulnerability affect Python packages?\n"
            "- Show critical advisories for npm\n\n"
    ...
    async def _search_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if not hits:
                await update.message.reply_text("No documents found. Try different keywords.")
                return

            # Deduplicate by source_id (since chunks may share the same source)
            seen_ids = set()
            unique_docs = []
            for hit in hits:
                source_id = hit.get("source_id", hit.get("ghsa_id", ""))
            ...
            # Format results
            response = f"Found {len(unique_docs)} documents:\n\n"
            for idx, hit in enumerate(unique_docs, 1):
    ...
    async def _handle_question(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if not chunks:
                await update.message.reply_text("No relevant documents found. Try rephrasing your question.")
                return
